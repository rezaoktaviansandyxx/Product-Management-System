...
const AddRole = ({ onSuccess, onClose }) => {
    ...
    useEffect(() => {
        ...
        const isNameValid = formData.name.trim().toLowerCase() !== 'administrator';
        
        setIsFormValid(requiredFieldsFilled && !hasErrors && isNameValid);
    }, [formData, errors]);
    ...
    const handleSubmit = (e) => {
        ...
        axios.post(AppConfig.API_URL + '/roles', roleData)
            ...
            .catch((error) => {
                Swal.fire({
                    title: 'Error!',
                    text: error.response?.data?.message || 'Something went wrong while saving the role.',
                    icon: 'error',
                    confirmButtonText: 'OK'
                });
            })
